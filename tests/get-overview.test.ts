...
describe('No import tokens in domains', () => {
  it('policies/AccessPolicyHelper.ts has no import-source tokens in its domains', () => {
    const domains = getFileDomains('AccessPolicyHelper');
    const hasImportSource = domains.some(dt => dt.sources.includes('import'));
    expect(hasImportSource).toBe(false);
  });

  it('query "auth" does NOT return policies/AccessPolicyHelper.ts (no longer indexed via imports)', () => {
    const paths = queryPaths('auth');
    expect(paths.some(p => p.includes('AccessPolicyHelper'))).toBe(false);
  });

  it('query "policy" DOES return policies/AccessPolicyHelper.ts (matched via own path/filename)', () => {
...
describe('Compound export name matching', () => {
  it('query "AccessPolicyHelper" (camelCase) finds policies/AccessPolicyHelper.ts', () => {
    const paths = queryPaths('AccessPolicyHelper');
    expect(paths.some(p => p.includes('AccessPolicyHelper'))).toBe(true);
  });
});
