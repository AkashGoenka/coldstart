...
// ============================================================================
// Test 1: Pluralization — query "profile" matches profiles/ProfileMenu.ts
// ============================================================================
describe('Pluralization', () => {

  it('query "profile" (singular) matches profiles/ProfileMenu.ts', () => {
    const paths = queryPaths('profile');
    expect(paths.some(p => p.includes('ProfileMenu'))).toBe(true);
  });

  it('query "profiles" (plural) also matches profiles/ProfileMenu.ts', () => {
    const paths = queryPaths('profiles');
    expect(paths.some(p => p.includes('ProfileMenu'))).toBe(true);
  });
});
...
describe('Import-only exclusion', () => {
  it('query "auth" does NOT return policies/AccessPolicyHelper.ts by default (import-only match)', () => {
    const paths = queryPaths('auth');
    expect(paths.some(p => p.includes('AccessPolicyHelper'))).toBe(false);
  });
});
...
describe('Import-only override', () => {
  it('query "auth policy" with include_import_only:true DOES return policies/AccessPolicyHelper.ts', () => {
    // "auth" matches AccessPolicyHelper via import only, "policy" matches via path+filename
    ...
    expect(paths.some(p => p.includes('AccessPolicyHelper'))).toBe(true);
  });

  it('query "auth policy" WITHOUT include_import_only does NOT return AccessPolicyHelper (import-only for auth)', () => {
    // "auth" is import-only for AccessPolicyHelper — if that is the ONLY concept that matched via
    ...
    // AccessPolicyHelper has "policy" via path/filename — so allImportOnly=false → it IS included
...
describe('All-common-token diagnostic', () => {
  it('returns a diagnostic when all matched tokens are common (high frequency)', () => {
    // "node" appears in AccessPolicyHelper — likely common in fixtures; use a very common token
    ...
    // Query "node" to match AccessPolicyHelper — "node" will appear in 1 file only.
