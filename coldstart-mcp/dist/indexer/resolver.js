...
async function loadTsConfigPaths(rootDir) {
    const paths = new Map();
...
async function resolveSpecifier(specifier, fromFile, fileIdSet, rootDir, aliasMap, language) {
    // ---------------------------------------------------------------------------
        ...
        if (!matched) {
            ...
            if (language === 'go') {
                ...
                if (await fileExists(candidate + '.go'))
                    return relative(rootDir, candidate + '.go').replace(/\\/g, '/');
            }
...
export async function resolveImports(files, rootDir) {
    ...
    for (let i = 0; i < files.length; i += BATCH) {
        ...
        await Promise.all(batch.map(async (file) => {
            for (const specifier of file.imports) {
                ...
                const resolved = await resolveSpecifier(spec, absFromFile, fileIdSet, rootDir, aliasMap, file.language);
