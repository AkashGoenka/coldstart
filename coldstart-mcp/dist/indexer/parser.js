...
import { LANGUAGE_CONFIGS, DOMAIN_KEYWORDS, ENTRY_POINT_NAMES, ARCH_ROLE_PATTERNS, } from '../constants.js';
 ...
 */
function extractContentTokens(content, language, existingExports) {
    ...
    for (const m of callMatches) {
        ...
        const name = m[1];
        if (/^(import|require|from|export|return|if|else|for|while|switch|catch|typeof|instanceof|new|delete|void|throw|await|async|function|class|const|let|var|console|log|warn|error)$/.test(name))
            continue;
    ...
    const exportTokens = new Set();
    for (const exp of existingExports) {
        for (const tok of tokenize(exp))
            exportTokens.add(tok);
    }
    for (const tok of exportTokens)
        termCounts.delete(tok);
...
export function buildFileId(relativePath) {
