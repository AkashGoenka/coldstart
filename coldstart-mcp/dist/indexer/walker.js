...
export async function walkDirectory(options) {
    const { rootDir, excludes = [], includes = [], maxFileSizeBytes = 1_000_000, } = options;
    ...
    async function walk(dir) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                ...
                if (includeSet) {
                    const isIncluded = [...includeSet].some(inc => fullPath.startsWith(inc) || inc.startsWith(fullPath));
                    if (!isIncluded)
                        continue;
                }
                await walk(fullPath);
