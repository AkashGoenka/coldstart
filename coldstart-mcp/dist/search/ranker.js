...
export function findFiles(query, index, options = {}) {
    ...
    for (const [id, file] of index.files) {
        if (options.domain && file.domain !== options.domain)
            continue;
        const reasons = [];
        let score = 0;
        ...
        const matchingTermCount = tokens.filter(tok => base.includes(tok) ||
            file.relativePath.toLowerCase().includes(tok) ||
            file.exports.some(e => e.toLowerCase().includes(tok)) ||
            file.domain.includes(tok)).length;
