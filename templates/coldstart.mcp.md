...
- `gs` ends with an `Edited together` section (repos with git history): files that keep changing in the same commits as this one, with the count as the evidence. It is a HABIT, not a dependency — there is no import or call edge, so don't go looking for one. It is often what an edit here forgets: a sibling implementation, a test in another language, a doc that has to stay in sync. It is shown in EVERY view, including a narrowed one — narrowing asks for less of this file, not for fewer channels. No section = no history yet (new file, shallow clone) or nothing pairs strongly enough.
- `gs` also lists `Related` files: files sharing a rare identifier or string token with this one where NO import edge connects them — migration↔model, config-by-name registration, cross-language (JS↔Python) pairs. The shared token is printed as the reason. Like `Edited together`, it is shown in EVERY view.
